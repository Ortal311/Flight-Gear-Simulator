...
public class hybridAlgorithm {
    ...
    TimeSeries tsZscore;
    TimeSeries tsZscoreAnomal;

    TimeSeries tsReg;
    TimeSeries tsRegAnomal;
    ...
    public void learnNormal(TimeSeries ts) {
       ...
       // ad.cf=cfmore95list;
        ...
        for (String att : cfmore95.keySet()){
            ...
            tsReg.tsNum.put(index++, ts.ts.get(att));
            tsReg.atts.add(att);
        }
        ...
        for (Map.Entry<String, CorrelatedFeatures> alg : cfmore95.entrySet()) {
            attALG.put(alg.getKey(), "Reggretion");
    ...
    public List<AnomalyReport> detect(TimeSeries ts) {
        ...
        for (String att : cfmore95.keySet()){
            ...
            tsRegAnomal.tsNum.put(index++, ts.ts.get(att));
...
//        timeStep.add()
        attribute1.addListener((ob, oldV, newV) -> {//to delete the old graph if attribute has changed
            attribute2.setValue(ad.getCorrelateFeature(attribute1.getValue()));
         //   if (attribute2.getValue() != null) {
